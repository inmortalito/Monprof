    ...
    return Container(
      child: FlatButton(
          ...
          onPressed: () async {
            ...
            if (x != null || res) {
              Provider.of<CoursesProvider>(context, listen: false).setProgress(
                  int.parse(widget.sections[0].sectionDetails.courseId),
                  fChecked,
                  x);
              _scaffoldKey.currentState.showSnackBar(
                  SnackBar(content: Text("Sections Marking Completed!")));
            } else {
              _scaffoldKey.currentState.showSnackBar(
                  SnackBar(content: Text("Sections Marking Failed!")));
            }
            setState(() {
              isLoadingMark = false;
            });
