...
class CartProducts with ChangeNotifier {
  ...
  void updateCart(
      List<CartModel> courses, List<int> ids, List<int> bundleIds, int type) {
    if (type == 0) {
...
class CartApiCall {
  ...
  Future<bool> removeFromCart(
      dynamic id, BuildContext ctx, CartModel detail) async {
  ...
  Future<bool> removeBundleFromCart(
      String bundleId, BuildContext ctx, CartModel detail) async {
    authToken = await storage.read(key: "token");
    CartProducts pro = Provider.of<CartProducts>(ctx, listen: false);
