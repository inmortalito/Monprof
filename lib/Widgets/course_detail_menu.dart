...
class CourseDetailMenuScreen extends StatefulWidget {
  ...
  CourseDetailMenuScreen(
      this.isFav, this.isPurchased, this.details, this.progress);
  @override
    ...
    return InkWell(
      onTap: () async {
        if (idx == 0) {
          ...
          bool x = await wish.toggle(widget.details.course.id, widget.isFav);
          if (x)
            setState(() {
      ...
      body: Container(
        ...
        child: Column(
          children: [
            ...
            if (!widget.isPurchased)
              menutiles(
                  widget.isFav ? "Remove from Favourite" : "Add to Favourite",
                  Icons.favorite,
                  3,
                  mode.txtcolor),
            menutiles(
